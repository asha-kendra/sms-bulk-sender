...
## Notes / things worth double-checking before sending a real blast

- `/bulk` has no auth check — anyone with the URL can trigger a send. Consider adding an API key check or restricting the Catalyst API Gateway route if this will be reachable outside your team.
- There's no de-duplication or rate limiting — a file with duplicate phone numbers will send duplicate texts, and large lists send all at once (`Promise.all` over the whole batch).
- Phone numbers aren't validated or normalized before being handed to Vumber.
